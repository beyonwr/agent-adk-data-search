...
class FileSource(BaseModel):
    ...
    #TODO 추가 필요
    # def resolve_segments(self, ctx: Context | None = None) -> list(LinePlotSegment):
    #df = pd.read_csv(self.path)
    #return build_segments_from_df(df, self.columns)

ChartSource = Union[ArtifactSource, FileSource, DirectSource]

class LineChartRequest(BaseModel):
    source: ChartSource = Field(..., discriminator="source_type")
    title: str = "Shared-X Line Plot"
    x_mdoe: Literal["concat", "align", "keep"] = "concat"
